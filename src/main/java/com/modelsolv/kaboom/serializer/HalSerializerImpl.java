...
public class HalSerializerImpl extends AbstractSerializerImpl {
    ...
    private boolean buildObjectRepresentation(Representation rep, Object obj, CanonicalObjectReader reader,
            ResourceDataModel model, Stack<Object> objectStack) {
        ...
        for (RDMProperty prop : props) {
            if (!(prop instanceof RDMReferenceProperty)) {
                ...
                if (prop instanceof ReferenceLink) {
                    // render link
                    Object targetObj = reader.getPropertyValue(obj, (ReferenceLink) prop);
                    if (targetObj instanceof Iterable<?>) {
                        Iterable<Object> targets = (Iterable<Object>) targetObj;
                        for (Object target : targets) {
                            buildLink(rep, target, reader, (ReferenceLink) prop);
                        }
                    } else {
                        buildLink(rep, targetObj, reader, (ReferenceLink) prop);
                    }
                } else {
                    // render an embedded object
                    ReferenceEmbed refEmbed = (ReferenceEmbed) prop;
                    ResourceDataModel embeddedModel = refEmbed.getEmbeddedModel();
                    ...
                    if (targetObject instanceof Iterable<?>) {
                        ...
                        for (Object targetElement : targetCollection) {
                            Representation embeddedRep = createNewRepresentation(targetElement, reader, embeddedModel);
                            if (buildObjectRepresentation(embeddedRep, targetElement, reader,
                                    refEmbed.getEmbeddedModel(), objectStack)) {
                                rep.withRepresentation(refEmbed.getName(), embeddedRep);
                            }
                        }
                    } else {
                        Representation embeddedRep = createNewRepresentation(targetObject, reader, embeddedModel);

                        if (buildObjectRepresentation(embeddedRep, targetObject, reader, refEmbed.getEmbeddedModel(),
                                objectStack)) {
                            rep.withRepresentation(refEmbed.getName(), embeddedRep);
                        }
                    }
