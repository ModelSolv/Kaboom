...
public class SerializerTest {
    ...
    private void verifyTaxFilingMessage(String message, boolean resourceRoot, boolean linkedReference,
        ...
        if (resourceRoot) {
            // check the self-link
            assertEquals("http://modelsolv.com/taxblaster/api/taxFilings/1234", root.at("/_links/self/href").asText());
        }
        if (linkedReference) {
            // Verify the reference link to Person.
            assertEquals("http://modelsolv.com/taxBlaster/api/people/555-33-5577", root.at("/_links/taxpayer/href")
                    .asText());
        } else {
            ...
            if (checkMultiValued) {
                ...
                assertEquals("425 Dobbs Industrial Court", personNode.at("/_embedded/addresses/1/street1").asText());
            }
            if (embeddedResource) {
    ...
    public void testSerializeEmbeddedResource() {
        ...
        TaxFiling filing = buildTaxFiling();
        taxFilingRDM.includingProperties("currency", "filingID", "grossIncome", "jurisdiction", "period",
                "taxLiability", "year").withReferenceEmbed(
                rdmFactory.createReferenceEmbed((CDMReferenceProperty) taxFilingType.getProperty("taxpayer"))
                        .withEmbeddedDataModel(personRDM.includingProperties("taxpayerID", "lastName", "firstName", "otherNames")));
        // Register both resource definitions, so Person will be linked by
        // default.
        registerTaxFilingORD();
        registerPersonORD();
